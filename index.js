...
module.exports = (options, ctx, globalCtx) => ({
    extendPageData($page) {
        try {
            const { html } = $page._context.markdown.render(
            ...
            for (const h of $page.headers || []) {
                const titlePlaintext = $page._context.markdown.renderInline(
                    h.title
                );
            ...
            if (!$page.title)
                $page.title = customTitles[normalizePath($page.path)];
        } catch (e) {
            // incorrect markdown
...
function getCustomTitles(globalCtx) {
    try {
        const sidebarConfig = _.get(
            globalCtx,
            "_pluginContext.themeConfig.sidebar"
        );
        ...
        console.error(
            "[fulltext-search] Error while getting sidebar paths:",
            e
        );
        return {};
    }
...
function normalizePath(rawPath) {
    ...
    try {
        ...
        console.error(
            `[fulltext-search] Error while normalizing path ${rawPath}:`,
            e
        );
        return null;
    }
